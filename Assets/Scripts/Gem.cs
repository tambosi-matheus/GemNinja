...
public class Gem : MonoBehaviour
{
    ...
    {
        AudioManager.Instance.Play(audioSource, "Spawn");
        UpdateBody();
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "BottomCollider")
            manager.OnGemTouched(this, bottomCollider: true);
        else
        {
            var value = Int16.Parse(name);
            PoolParticle.Instance.Dequeue(transform.position, value);
            manager.OnGemTouched(this);
        }
    }
}
